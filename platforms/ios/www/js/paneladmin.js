            ...
            var aIndicadores = JSON.parse(respuesta);
             $.each( aIndicadores, function( i, value ) {
                ...
                if(parseInt(value["reservas"])>0){
                    $("#menuadmin").html('<center><button onclick="irareservas()" class="btn-burbit-green" ><span class="icon-checkmark"></span></span> '+value["reservas"]+' Cambios Hoy</button></center>');
                }else{
                    $("#menuadmin").html('<center><button onclick="irareservas()" class="btn-burbit-grey" ><span class="icon-checkmark"></span></span> Sin Cambios Hoy</button></center>');
                }
                if(parseInt(value["vistaswod"])>0){
                    $("#menuadmin").append('<center><button onclick="iramenuwod()" class="'+value["colorwod"]+'" ><span class="icon-rocket"></span></span> '+value["vistaswod"]+' Vistas de la INFO</button></center>');
                }else{
                    $("#menuadmin").append('<center><button onclick="iramenuwod()" class="'+value["colorwod"]+'" ><span class="icon-rocket"></span></span> '+value["vistaswod"]+' Vistas de la INFO</button></center>');
                }
                ...
                if(parseInt(value["totalmegustas"])>0){
                    $("#menuadmin").append('<center><button onclick="vermegustas()" class="btn-burbit-green" ><span class="icon-heart"></span></span> '+value["totalmegustas"]+' Me Gusta en la INFO</button></center>');
                }else{
                    $("#menuadmin").append('<center><button onclick="vermegustas()" class="btn-burbit-grey" ><span class="icon-heart"></span></span> Aun No Gusta la INFO</button></center>');
                }
